...
def personal_statistics():
    ...
    if not from_date:
        ...
        from_date_obj = datetime.now() - timedelta(days=6)  # 6 days ago + today = 7 days
    ...
    gitlab_stats = get_gitlab_personal_stats(gitlab_username, from_date, to_date)  # Now includes app-interface
...
def get_github_personal_stats(username, from_date, to_date):
    ...
    for repo_name in repos_contributed:
        org_name = repo_organizations.get(repo_name, username)  # Default to username if not found
        ...
        if org_name == username:
            ...
            organization_stats[org_name]["repo_stats"][repo_name] = 0  # Initialize PR count for this repo
...
def get_gitlab_personal_stats(username, from_date, to_date):
    ...
    try:
        ...
        if isinstance(app_interface_data, dict) and "data" in app_interface_data:
            ...
            # Fallback if data structure is different
            app_interface_merged_mrs = app_interface_data if isinstance(app_interface_data, list) else []
                ...
                if html_url:
                    ...
                    if len(url_parts) >= 5 and "gitlab" in url_parts[2]:
                        ...
                        if len(url_parts) > domain_index + 1:
                            org_name = url_parts[domain_index + 1]  # First path segment is usually the group
    ...
    for repo_name in repos_contributed:
        ...
        if org_name == username:
            ...
            if org_name not in organization_stats:
                organization_stats[org_name] = {"repos": [], "repo_stats": {}, "pr_count": 0}
            organization_stats[org_name]["repos"].append(repo_name)
            organization_stats[org_name]["repo_stats"][repo_name] = 0  # Initialize MR count for this repo
...
def get_jira_personal_stats(username, from_date, to_date):
    ...
    if not username or username in ["Not available", "Unable to fetch user info"]:
        return {"closed_tickets_count": 0, "closed_tickets": []}
    ...
    # Filter tickets by user (by assignee display name) and date range
    user_tickets = []
    ...
    for ticket in jira_closed_tickets:
        ...
        try:
            ...
            resolved_date = datetime.fromisoformat(resolved_at.replace('Z', '+00:00')).date()
            from_date_obj = datetime.strptime(from_date, "%Y-%m-%d").date()
            to_date_obj = datetime.strptime(to_date, "%Y-%m-%d").date()
